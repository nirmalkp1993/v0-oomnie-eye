...
export function isCameraTab(tab: AppTab): boolean {
  return (
    tab === "camera" || tab === "camera-groups" || tab === "camera-recording"
  );
}
  ...
  const renderTabButton = (
    ...
    const btn = (
      ...
      >
        ...
        />
        {expanded && (
          <span className={cn("text-sm", nested && "text-[13px]")}>
            {label}
          </span>
        )}
      </button>
      ...
      >
        ...
        <nav className="flex flex-1 flex-col gap-1 px-2 py-1">
          {topMenuItems.map((item) =>
            renderTabButton(item.tab, item.icon, item.label),
          )}
            ...
            <Collapsible open={cameraOpen} onOpenChange={setCameraOpen}>
              <CollapsibleTrigger asChild>
                ...
                >
                  ...
                  <ChevronDown
                    className={cn(
                      "size-4 shrink-0 text-chrome-icon-inactive",
              ...
              <CollapsibleContent className="space-y-0.5 pt-0.5">
                {cameraChildren.map((c) =>
                  renderTabButton(c.tab, c.icon, c.label, true),
                )}
              </CollapsibleContent>
            </Collapsible>
          )}
