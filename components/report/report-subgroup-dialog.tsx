...
export function ReportSubgroupDialog() {
  const {
  ...
  const parent = useMemo(
    () => reportGroups.find((g) => g.id === subgroupModalParentId) ?? null,
    [reportGroups, subgroupModalParentId],
  )
      ...
      <DialogContent className="max-w-sm border-border bg-card">
        ...
        <form onSubmit={submit} className="space-y-3">
          <div className="space-y-2">
            <Label htmlFor="report-subgroup-name" className="text-accent">
              Subgroup name
            </Label>
            <Input
              id="report-subgroup-name"
              value={name}
